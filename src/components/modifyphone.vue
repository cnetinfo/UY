  ...
  <div>
    <x-dialog v-model="showNoScroll" class="dialog-demo" :scroll="false">
      <div style="padding:10px;">
        <h3 style="font-size: 16px; font-weight: normal;">手机修改</h3>
        <group class="m-t-10">
          <x-input title="新手机" placeholder="请输入新的手机号码"  :max="11" v-model="phone"></x-input>
      ...
      <div  style="display: flex;">
        <button class="colortbtn wp50" style="border-radius:0px;" @click="bindphone">确 定</button>
        <button class="defaultbtn colorborder wp50" style="border-radius:0px;" @click="showMD">取 消</button>
      </div>
    </x-dialog>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    methods:{
      showMD(){
        this.$parent.isupdatephone = false;
      },
      bindphone(){
        if(this.phone == ''){
        ...
        let l = 'mall/resetphonenumberV1.do';
        let d = {
          phoneNumber:this.phone,
          verifyCode:this.verifycode
        };
        this.$post(l,d,function(_t,r){
          _t.$vux.toast.text('修改成功','middle');
      ...
      sendCode(){
        if(!this.start){
          ...
          let l = "mall/getVerifyCodeForChangePhoneV1.do";
          this.$post(l,d,function(_t,r){
            setTimeout(() => {
              _t.start = true;
              _t.$vux.loading.hide();
